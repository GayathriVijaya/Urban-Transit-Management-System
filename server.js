    ...
    if (role === 'driver') {
        db.get('SELECT * FROM operator_personal WHERE Name = ? AND op_password = ?', [username, password], (err, row) => {
            ...
            if (row) {
                ...
                return res.redirect('/main.html'); // Redirect to driver dashboard
            } else {
        ...
        db.get('SELECT * FROM Admin WHERE Admin_name = ? AND Ad_password = ?', [username, password], (err, row) => {
            ...
            if (row) {
                ...
                return res.redirect('/admin_dashboard.html'); // Redirect to admin dashboard
            } else {
                return res.status(401).send('Invalid credentials');
            }
        });
...
//         const countQuery = `SELECT COUNT(*) as feedbackCount FROM Feedback`;
//         const result = await db.get(countQuery); // Replace 'db.get' based on your DB driver

//         // Send the count as a response
//         res.json({ feedbackCount: result.feedbackCount });
//     } catch (error) {
